...
namespace rijndael {
	namespace {
		...
		public:
			...
			[[gsl::suppress(gsl.view)]] transcoder(const constant_table& c, key_view k) noexcept : transcoder {}
			{
				apply_key(c, k);
			}
		...
		private:
			...
			[[gsl::suppress(bounds)]] [[gsl::suppress(gsl.view)]] static void
			inverse_mix_column(const constant_table& table, column c) noexcept
			{
			...
			[[gsl::suppress(bounds)]] [[gsl::suppress(gsl.view)]] void
